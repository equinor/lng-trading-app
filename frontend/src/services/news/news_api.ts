...
export async function patchFavourite(articleId: number, favourited: boolean) {
  await apiRequest<{ ok: boolean }>(`${NEWS_BASE}/${encodeURIComponent(String(articleId))}/favourite`, {
    method: "PATCH",
    body: { favourited },
  })
}
...
export async function patchSentiment(articleId: number, official_sentiment: DbSentiment) {
  await apiRequest<{ ok: boolean }>(`${NEWS_BASE}/${encodeURIComponent(String(articleId))}/sentiment`, {
    method: "PATCH",
...
export async function patchClassification(articleId: number, category: string[], region: string[]) {
  await apiRequest<{ ok: boolean }>(`${NEWS_BASE}/${encodeURIComponent(String(articleId))}/classification`, {
    method: "PATCH",
